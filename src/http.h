...
namespace vm
{
    ...
    struct http_connection : tcp_connection
    {
        ...
        void query(http_request requests,
    ...
    private:
        bool is_alive = false;
        std::map<int, std::function<void(vm::http_response)>> query_back;
    ...
    struct http_server
    {
        ...
        void set_on_disconnect(std::function<void(http_connection&)> con);

    private:
        tcp_server<http_connection> server;
        std::function<void(http_connection&, http_request)> on_request;
        std::function<void(http_connection&)> on_disconnect;
